...
final class Role private(val id: String) extends AnyVal {
  ...
  override def toString: String = Role.encodeString(this)
 ...
 */
object Role extends ValueCompanion[Role] {
   ...
   * @return A new role.
   */
  def apply(id: String): Role = decodeString(id)
  ...
  /* Decode a role. */
  override protected def decode(string: String): Role = new Role(string)

  /* Encode the role. */
  override protected def encode(value: Role): String = value.id
