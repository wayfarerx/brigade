...
package net.wayfarerx.brigade.main
...
import net.wayfarerx.brigade.service.{Connection, Storage}
...
object BrigadeMain {
  ...
  def main(args: Array[String]): Unit =
    if (args.length != 0 && args.length != 2) {
      ...
      println("  The Discord login token must be provided on the system input stream: cat token.txt | brigade ...")
      ...
      var result = 1
      try {
        val system = ActorSystem("brigade")
